...
/******************************** PART 1 **************************************/
	    ...
	    case WAIT:
		/* 
		* Grab the current thread control block (TCB) and let the current thread
		* sleep in the waitQueue while passing the condition that this thread's 
		* ID is equal to the condition being passed in through getTid().
		*/	    
	    ...
	    case EXIT:
		/* 
		* Grab the current thread control block (TCB) and search the waitQueue
		* for a thread that is waiting for the parent's ID condition with the 
		* thread's own ID. At the end, delete this thread from the scheduler.
		*/	    
            myTcb = scheduler.getMyTcb(); 
...
/******************************** PART 2 **************************************/
	    ...
	    case RAWREAD: // read a block of data from disk
			while ( disk.read( param, ( byte[] )args ) == false )
			    ioQueue.enqueueAndSleep( COND_DISK_REQ ); // relinquish CPU 
			while ( disk.testAndResetReady( ) == false )
			    ioQueue.enqueueAndSleep( COND_DISK_FIN ); // relinquish CPU
		return OK;
